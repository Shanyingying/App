...
.controller('listCtrl',function($scope){
    ...
    $scope.delete_item=function(item){
      ...
      $scope.items.splice(idx,1);//删除掉一个
    };
  //  splice第一个参数为：添加或者删除项目的位置；第二个参数为：要删除的项目数量0为不删除；第三个参数为:(可选)向数组添加新项目
  //排序，先把自己删除掉，再把自己插入到鼠标移到的位置上
    $scope.move_item = function(item, fromIndex, toIndex) {
      $scope.items.splice(fromIndex, 1);//从这个位置上删掉一个
      $scope.items.splice(toIndex, 0, item);//从目标位置上插入就是拉到哪
    };
  })
/*幻灯片的控制器*/
